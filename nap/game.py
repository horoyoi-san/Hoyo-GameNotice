...
import nap.util as util
import nap.data as data
...
async def game(settings) -> tuple[bool, list[dict]]:
    ...
    url = f'https://sg-announcement-api.hoyoverse.com/common/nap_global/announcement/api/getAnnContent?game=nap&game_biz=nap_global&lang={lang}&bundle_id=nap_global&level=60&platform=pc&region=prod_gf_jp&uid=1'
    ...
    for ann in ann_list:
        ...
        added_list.append(
            f'[{ann_content.get("title")}](log/nap/{ann.get("ann_id")}.md)')
        ...
        for sec in sections:
            ...
            if len(sec_split) > 3:
                embed["fields"].append(
                    {"name": "", "value": f'[see more...](https://github.com/{repo}/tree/main/log/nap/{ann.get("ann_id")}.md)'}
                )
