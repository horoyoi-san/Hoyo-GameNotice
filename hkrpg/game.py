...
async def game(settings) -> tuple[bool, list[dict]]:
    ...
    url = f'https://sg-hkrpg-api.hoyoverse.com/common/hkrpg_global/announcement/api/getAnnContent?game=hkrpg&game_biz=hkrpg_global&lang={lang}&bundle_id=hkrpg_global&level=80&platform=pc&region=prod_official_cht&uid=1'
    ...
    for ann in ann_list:
        ...
        for sec in sections:
            ...
            if len(sec_split) > 3:
                embed["fields"].append(
                    {"name": "", "value": f'[see more...](https://github.com/{repo}/tree/main/log/hkrpg/{ann.get("ann_id")}.md)'}
                )
